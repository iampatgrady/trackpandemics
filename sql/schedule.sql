  ...
  WITH
    raw_data AS (
      SELECT
        ...
        SPLIT(SPLIT(SPLIT(httpRequest.requestUrl, "?")[SAFE_OFFSET(1)],"&")[SAFE_OFFSET(3)],"=")[SAFE_OFFSET (1)] AS zipcode,
      FROM
...
WHEN NOT MATCHED THEN
    INSERT (timestamp, receiveTimestamp, requestSize, responseSize, ip_hash, cacheHit, isFahrenheit, temperature, zipcode, long, lat, geopoint)
        VALUES (timestamp, receiveTimestamp, requestSize, responseSize, ip_hash, cacheHit, isFahrenheit, temperature, zipcode, long, lat, geopoint)
